...
| サービス | 責務 | 主要メソッド |
|----------|------|------------|
| **PlanningService** | 学期ごとの基本練習計画作成、練習内容テンプレート管理、テンプレート適用、計画制約定義 | createBasePlan(), managePracticeTemplates(), applyTemplates(), defineConstraints() |
| **TemplateManager** | 練習テンプレートの作成と管理、整合性検証、テンプレートの分類 | createTemplate(), validateTemplate(), categorizeTemplates() |
| **ScheduleGenerator** | 基本計画と日程から最適なスケジュールを生成、品質評価、制約適用 | generateOptimalSchedule(), evaluateScheduleQuality(), applyConstraints() |
| **Rescheduler** | 欠席情報を考慮したスケジュール再生成、変更最小化、パート優先度考慮 | recalculateWithAbsences(), minimizeChanges(), prioritizeParts() |
| **RotationEngine** | 監督者の公平な割り当て計算、公平性スコア評価、最適配置 | assignSupervisors(), calculateFairnessScore(), optimizeRotation() |
| **ConflictDetector** | 時間的競合検出、会場収容違反チェック、ローテーションルール検証 | detectTimeConflicts(), checkCapacityViolations(), validateRotationRules() |
| **AbsenceManager** | 欠席申請処理とワークフロー管理、影響分析、代替日程提案、関係者通知 | processRequest(), analyzeImpact(), suggestAlternatives(), notifyStakeholders() |
| **QualityAssurance** | 練習セッションの質評価、改善案提案、長期的な品質トレンド追跡 | evaluatePracticeQuality(), recommendImprovements(), trackQualityTrends() |
| **PracticeAnalytics** | 練習データの傾向分析、データに基づく洞察生成、最適化提案、出席予測 | analyzeTrends(), generateInsights(), recommendOptimizations(), predictAttendance() |
| **VenueAllocator** | 練習セッションに最適な会場割り当て、会場利用最適化、会場変更対応、設備チェック | allocateVenues(), optimizeSpaceUsage(), handleVenueChanges(), checkEquipmentAvailability() |
| **MemberBalancer** | パート間のメンバー数バランス調整、最適移動提案、分布評価、スキルギャップ分析 | balancePartSizes(), suggestTransfers(), evaluateDistribution(), calculateSkillGaps() |
| **PermissionEnforcer** | アクセス権限のバリデーション、編集ルール適用、変更履歴監査、バージョン管理 | validateAccess(), enforceEditRules(), auditChanges(), manageVersions() |
| **PDFExporter** | スケジュールのPDF変換、練習シート生成、サマリーレポート作成、書式設定適用 | createPDF(), generatePracticeSheet(), createSummaryReport(), applyFormatting() |
| **VersionManager** | 公開スケジュールのバージョン管理、変更履歴追跡、過去バージョン取得、バージョン比較 | createVersion(), trackChanges(), retrieveVersion(), compareVersions() |
| **NotificationService** | 通知の送信管理、リマインダーのスケジューリング、配信状況追跡、失敗通知処理 | sendNotification(), scheduleReminders(), trackDeliveryStatus(), handleFailures() |
...
1. **基本計画管理**
   ...
   - **ManagePracticeTemplates(practiceTemplates)**
     - 入力: 練習テンプレートデータ
     - 振る舞い: `TemplateManager` → テンプレートの検証、登録、更新
     - 出力: 更新済テンプレートリスト

2. **練習表初期作成**
   - **GenerateInitialSchedule(basePlan, dateRange)**
     - 入力: 基本計画、日程範囲
     - 振る舞い: `ScheduleGenerator` の初期生成
     - 出力: 初期練習表案（未最適化）
   - **ApplyPracticeConstraints(scheduleId, constraints)**
     - 入力: スケジュールID、制約条件リスト
     - 振る舞い: `ScheduleGenerator` → 制約適用
     - 出力: 制約適用済みスケジュール
...
3. **公平性評価ダッシュボード**
   ...
   - PracticeAnalyticsによる練習データ傾向分析と洞察
   - 最適化提案の表示
   - 出席予測に基づく配置調整案
...
- **プラットフォーム**: Supabase
  ...
  - **Storage**: ファイルストレージ
  - **Realtime**: リアルタイム更新機能
  - **Functions**: サーバーレスAPI機能
  - **Row Level Security**: データアクセス制御

### 11.3 バッチ処理
- **スケジューラー**: Supabase Edge Functions
  - スケジュール生成ロジック
  - 定期処理
  - メール通知
...
- **ホスティング**: 
  - フロントエンド: Vercel
  - バックエンド: Supabase（マネージドサービス）
- **CI/CD**: GitHub Actions（シンプル設定）
- **モニタリング**: 
  - Supabase ダッシュボード
  - Vercel Analytics
