        ...
        {commentThreads.map((commentThread) => {
          ...
          try {
            if (commentThread.body) {
              const parsedBody = JSON.parse(commentThread.body);
              if (Array.isArray(parsedBody) && parsedBody.length > 0) {
                const firstBlock = parsedBody[0];
                if (firstBlock.content) {
                  if (
                    ...
                    body = firstBlock.content[0]?.text || '';
                  } else if (typeof firstBlock.content === 'string') {
                    // 旧形式: content は文字列
                    body = firstBlock.content;
                  }
                }
              }
            }
          } catch (error) {
            console.error('タイムラインのJSONパースエラー:', error);
            body = commentThread.body || '(No content)';
          }
