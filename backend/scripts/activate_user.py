...
def activate_user(email: str):
    ...
    # Supabase接続
    url = os.getenv("SUPABASE_URL", "https://your-project-ref.supabase.co")
    ...
    try:
        ...
        for user in users:
            user_email = getattr(
                user, "email", user.get("email") if isinstance(user, dict) else None
            )
            if user_email == email:
                target_user = user
                break
        ...
        user_email = getattr(
            target_user,
            "email",
            target_user.get("email") if isinstance(target_user, dict) else None,
        )
        user_id = getattr(
            target_user,
            "id",
            target_user.get("id") if isinstance(target_user, dict) else None,
        )
        confirmed_at = getattr(
            target_user,
            "email_confirmed_at",
            (
                target_user.get("email_confirmed_at")
