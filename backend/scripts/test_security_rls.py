...
class SecurityRLSTester:
    ...
    def make_request(self, endpoint: str, headers: Dict[str, str] = None) -> Dict[str, Any]:
    ...
    def test_users_table_access(self):
        ...
        if result["status_code"] == 200:
            print(f"    ❌ Anonymous access should be restricted, but got {len(result['data'])} users")
        else:
            print(f"    ✅ Anonymous access properly restricted (status: {result['status_code']})")
        ...
        if result["status_code"] == 200:
            print(f"    ✅ Service role can access all users ({len(result['data'])} users)")
        else:
            print(f"    ❌ Service role access failed (status: {result['status_code']})")
    ...
    def test_user_profiles_table_access(self):
        ...
        if result["status_code"] == 200:
            print(f"    ❌ Anonymous access should be restricted, but got {len(result['data'])} profiles")
        else:
            print(f"    ✅ Anonymous access properly restricted (status: {result['status_code']})")
    ...
    def test_venues_table_access(self):
        ...
        if result["status_code"] == 200:
            print(f"    ❌ Anonymous access should be restricted, but got {len(result['data'])} venues")
        else:
            print(f"    ✅ Anonymous access properly restricted (status: {result['status_code']})")
    ...
    def test_departments_table_access(self):
        ...
        if result["status_code"] == 200:
            print(f"    ❌ Anonymous access should be restricted, but got {len(result['data'])} departments")
        else:
            print(f"    ✅ Anonymous access properly restricted (status: {result['status_code']})")
    ...
    def test_auth_users_protection(self):
        ...
        if result["status_code"] == 404:
            ...
            print(f"    ❌ auth.users table should be protected, but got status: {result['status_code']}")
    
    def test_service_role_full_access(self):
        """サービスロールが全テーブルにアクセスできるかをテスト"""
        print("🔒 Testing service role full access...")
        
        tables = ["users", "user_profiles", "user_roles", "venues", "departments", "availability_slots", "venue_attributes", "recurring_units"]
        headers = {
            "apikey": self.service_role_key,
            "Authorization": f"Bearer {self.service_role_key}"
        }
        ...
        for table in tables:
            ...
            if result["status_code"] == 200:
                print(f"    ✅ Service role can access {table} ({len(result['data'])} records)")
            else:
                print(f"    ❌ Service role cannot access {table} (status: {result['status_code']})")
    ...
    def test_remote_connection(self):
        ...
        if result["status_code"] == 200:
            ...
            print(f"    ❌ Failed to connect to remote project (status: {result['status_code']})")
