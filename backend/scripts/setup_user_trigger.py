...
def setup_user_sync_trigger():
    ...
    try:
        ...
        for auth_user in users_list:
            user_email = getattr(auth_user, 'email', auth_user.get('email') if isinstance(auth_user, dict) else None)
            user_id = getattr(auth_user, 'id', auth_user.get('id') if isinstance(auth_user, dict) else None)
