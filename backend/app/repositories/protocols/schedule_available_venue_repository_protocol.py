...
class ScheduleAvailableVenueRepositoryProtocol(Protocol):
    ...
    async def find_all_with_details(self, limit: int = 100, offset: int = 0, schedule_id: UUID | None = None, venue_id: UUID | None = None) -> list[dict[str, Any]]: ...
    async def count_all(self, schedule_id: UUID | None = None, venue_id: UUID | None = None) -> int: ...
    ...
    async def find_by_schedule_and_venue(self, schedule_id: UUID, venue_id: UUID) -> dict[str, Any] | None: ...
    async def create(self, schedule_venue_data: dict[str, Any]) -> dict[str, Any]: ...
    async def update(self, schedule_venue_id: UUID, update_data: dict[str, Any]) -> dict[str, Any]: ...
