...
class AttendanceService:
    ...
    def get_all_attendances(self) -> list[dict[str, Any]]:
        """すべての出欠記録を取得"""
        return self.repository.find_all()

    def get_attendance(self, attendance_id: UUID) -> dict[str, Any]:
        """指定したIDの出欠記録を取得"""
        attendance = self.repository.find_by_id(attendance_id)
        if not attendance:
            raise APIException(ErrorMessage.USER_NOT_FOUND)
        return attendance

    def get_attendances_by_practice(self, practice_schedule_id: UUID) -> list[dict[str, Any]]:
        """指定した練習スケジュールの出欠記録を取得"""
        return self.repository.find_by_practice_schedule(practice_schedule_id)

    def get_attendances_by_user(self, user_id: UUID) -> list[dict[str, Any]]:
        """指定したユーザーの出欠記録を取得"""
        return self.repository.find_by_user(user_id)

    def get_attendance_by_practice_and_user(
        self, practice_schedule_id: UUID, user_id: UUID
    ) -> dict[str, Any] | None:
        """指定した練習とユーザーの組み合わせの出欠記録を取得"""
        return self.repository.find_by_practice_and_user(practice_schedule_id, user_id)

    def create_attendance(self, attendance_data: dict[str, Any]) -> dict[str, Any]:
        """出欠記録を作成"""
        # 既存の記録があるかチェック
        existing = self.repository.find_by_practice_and_user(
            attendance_data["practice_schedule_id"], attendance_data["user_id"]
        )
        if existing:
            raise APIException("既にこの練習の出欠記録が存在します")

        return self.repository.create(attendance_data)

    def update_attendance(self, attendance_id: UUID, attendance_data: dict[str, Any]) -> dict[str, Any]:
        """出欠記録を更新"""
        # 存在チェック
        existing = self.repository.find_by_id(attendance_id)
        if not existing:
            raise APIException(ErrorMessage.USER_NOT_FOUND)

        return self.repository.update(attendance_id, attendance_data)

    def upsert_attendance(self, attendance_data: dict[str, Any]) -> dict[str, Any]:
        """出欠記録を作成または更新（practice_schedule_id + user_idの組み合わせで）"""
        return self.repository.upsert(attendance_data)

    def remove_attendance(self, attendance_id: UUID) -> bool:
        """出欠記録を削除"""
        attendance = self.repository.find_by_id(attendance_id)
        if not attendance:
            raise APIException(ErrorMessage.USER_NOT_FOUND)
    ...
    def get_attendance_summary(self) -> list[dict[str, Any]]:
        """練習別の出欠サマリーを取得"""
        return self.repository.get_attendance_summary()

    def get_user_attendance_history(self) -> list[dict[str, Any]]:
        """ユーザー別の出欠履歴を取得"""
        return self.repository.get_user_attendance_history()
    ...
    def bulk_update_attendances(
        ...
        for attendance_data in attendances:
            ...
            result = self.repository.upsert(attendance_data)
    ...
    def get_users_with_attendance_for_admin(
        ...
        if practice_schedule_id:
            attendances = self.repository.find_by_practice_schedule(practice_schedule_id)
