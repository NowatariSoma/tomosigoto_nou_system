...
class MemberAssignmentService:
    ...
    def get_assignment_by_id(self, assignment_id: UUID) -> dict[str, Any]:
        """IDでメンバー所属を取得"""
    ...
    def get_assignments_by_part(self, part_id: UUID) -> list[dict[str, Any]]:
        """パートIDでメンバー所属を取得"""
        # パートの存在確認
        part = self.part_repository.find_by_id(part_id)
        if not part:
            raise APIException(ErrorMessage.PART_NOT_FOUND)

        return self.repository.find_by_part_id(part_id)

    def get_assignments_by_user(self, user_id: str) -> list[dict[str, Any]]:
        """ユーザーIDでメンバー所属を取得"""
        # ユーザーの存在確認
        ...
        return self.repository.find_by_user_id(UUID(user_id))
    ...
    def get_assignments_with_details(
        ...
        return self.repository.find_with_details(assignment_id, part_id, user_uuid)

    def create_assignment(self, assignment_data: dict[str, Any]) -> dict[str, Any]:
        """メンバー所属を作成"""
        # ユーザーの存在確認
        if "user_id" in assignment_data:
            user = self.user_repository.find_by_id(assignment_data["user_id"])
            if not user:
                raise APIException(ErrorMessage.USER_NOT_FOUND)
        ...
        if "user_id" in assignment_data and "part_id" in assignment_data:
            ...
            existing_assignment = self.repository.find_by_user_and_part(user_uuid, part_uuid)
        ...
        # リポジトリを通してDBに保存
        created_assignment = self.repository.create(assignment_data)
    ...
    def update_assignment(
        ...
        """メンバー所属情報を更新"""
        # メンバー所属の存在確認
        existing_assignment = self.repository.find_by_id(assignment_id)
        ...
        # リポジトリを通して更新
        updated_assignment = self.repository.update(assignment_id, update_data)
    ...
    def remove_assignment(self, assignment_id: UUID) -> bool:
        ...
        # リポジトリを通してDBから削除
        self.repository.delete(assignment_id)
        logger.info(f"Member assignment deleted successfully: {assignment_id}")
        return True

    def remove_assignment_by_user_and_part(self, user_id: str, part_id: UUID) -> bool:
        ...
        existing_assignment = self.repository.find_by_user_and_part(user_uuid, part_id)
        if not existing_assignment:
            raise APIException(ErrorMessage.MEMBER_ASSIGNMENT_NOT_FOUND)

        # リポジトリを通してDBから削除
        self.repository.delete_by_user_and_part(user_uuid, part_id)
    ...
    def bulk_assign_to_part(self, part_id: UUID, user_assignments: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """パートに複数のユーザーを一括所属させる"""
        # パートの存在確認
        part = self.part_repository.find_by_id(part_id)
        if not part:
        ...
        for user_assignment in user_assignments:
            try:
                ...
                # 個別に作成
                created_assignment = self.create_assignment(assignment_data)
