...
class MaterialsPlaylistService:
    ...
    def get_all_materials_playlists(self) -> list[dict[str, Any]]:
        """すべてのプレイリストを取得"""
        return self.materials_playlist_repository.find_all()

    def get_materials_playlist_by_id(self, playlist_id: UUID) -> dict[str, Any]:
        """指定したIDのプレイリストを取得"""
        return self.materials_playlist_repository.find_by_id(playlist_id)

    def create_materials_playlist(self, playlist_data: dict[str, Any]) -> dict[str, Any]:
        """プレイリストを作成"""
        return self.materials_playlist_repository.create(playlist_data)

    def update_materials_playlist(self, playlist_id: UUID, playlist_data: dict[str, Any]) -> dict[str, Any]:
        """プレイリストを更新"""
        return self.materials_playlist_repository.update(playlist_id, playlist_data)

    def delete_materials_playlist(self, playlist_id: UUID) -> bool:
        """プレイリストを削除"""
        return self.materials_playlist_repository.delete(playlist_id)

    def search_materials_playlists(
        self,
        title: str | None = None,
        name: str | None = None,
        year: int | None = None
        ...
        """プレイリストを検索"""
        return self.materials_playlist_repository.search(
            title=title,
            name=name,
            year=year
        )
...
class MaterialsSubPlaylistService:
    ...
    def __init__(
        ...
        youtube_oauth_token_repository: YoutubeOauthTokenRepositoryProtocol,
    ):
        ...
        self.materials_playlist_repository = materials_playlist_repository
        self.youtube_oauth_token_repository = youtube_oauth_token_repository
        self.youtube_scopes = [
            'https://www.googleapis.com/auth/youtube.readonly'
        ]

    def get_all_materials_sub_playlists(self, playlist_id: UUID) -> list[dict[str, Any]]:
        """指定されたプレイリストのサブプレイリストを取得"""
        return self.materials_sub_playlist_repository.find_all(playlist_id)

    def get_materials_sub_playlist_by_id(self, playlist_id: UUID, sub_playlist_id: UUID) -> dict[str, Any]:
        """指定したIDのサブプレイリストを取得"""
        return self.materials_sub_playlist_repository.find_by_id(playlist_id, sub_playlist_id)
    ...
    def _get_system_credentials(self) -> Credentials | None:
        """
        ...
        """
        try:
            token_data = self.youtube_oauth_token_repository.find_system_token()
            ...
            if creds.expired and creds.refresh_token:
                try:
                    ...
                    # 更新されたトークンをDBに保存
                    self.youtube_oauth_token_repository.update_token(
                        token_data["id"],
                        {
    ...
    def _get_videos_from_playlist(
        ...
        if not credentials:
            credentials = self._get_system_credentials()
    ...
    def _update_sub_playlist_thumbnail_from_videos(
        self,
        ...
        try:
            # サブプレイリストに紐づく動画を取得
            videos = self.materials_video_repository.find_all(sub_playlist_id)
            ...
            # サムネイルが見つかった場合のみ更新
            if thumbnail_url:
                self.materials_sub_playlist_repository.update(
                    playlist_id,
                    sub_playlist_id,
    ...
    def _update_playlist_thumbnail_from_sub_playlists(
        self,
        playlist_id: UUID
    ) -> None:
        """
        ...
        try:
            ...
            sub_playlists = self.materials_sub_playlist_repository.find_all(playlist_id)
            ...
            # サムネイルが見つかった場合のみ更新
            if thumbnail_url:
                self.materials_playlist_repository.update(
                    playlist_id,
                    {"thumbnail_url": thumbnail_url}
                )
    ...
    def _import_videos_from_playlist(
        self,
        playlist_url: str,
        sub_playlist_id: UUID,
        ...
        try:
            ...
            videos = self._get_videos_from_playlist(playlist_url)
            ...
            for video in videos:
                ...
                existing = self.materials_video_repository.find_by_sub_playlist_and_url(
                    sub_playlist_id, video_url
                )
                ...
                try:
                    self.materials_video_repository.create(
                        sub_playlist_id,
                        video_data
                    )
                    imported_count += 1
            ...
            try:
                ...
                sub_playlist_info = self.materials_sub_playlist_repository.find_playlist_id_by_sub_playlist(
                    sub_playlist_id
                )
                if sub_playlist_info:
                    playlist_id = UUID(sub_playlist_info["playlist_id"])
                    # サブプレイリストのサムネイルを更新
                    self._update_sub_playlist_thumbnail_from_videos(sub_playlist_id, playlist_id)
                    # 親プレイリストのサムネイルも更新
                    self._update_playlist_thumbnail_from_sub_playlists(playlist_id)
            except Exception as e:
    ...
    def create_materials_sub_playlist(
        ...
        """
        # サブプレイリストを作成
        sub_playlist = self.materials_sub_playlist_repository.create(
            playlist_id,
            sub_playlist_data
        )
        ...
        if playlist_url and auto_import_videos:
            ...
            # 動画をインポート
            try:
                import_result = self._import_videos_from_playlist(
                    playlist_url,
                    sub_playlist_id,
                    recorded_date_str
                )
    ...
    def update_materials_sub_playlist(
        ...
        """
        # 既存のサブプレイリストを取得
        existing_sub_playlist = self.materials_sub_playlist_repository.find_by_id(
        ...
        # サブプレイリストを更新
        updated_sub_playlist = self.materials_sub_playlist_repository.update(
            playlist_id,
            sub_playlist_id,
            sub_playlist_data
        )
    ...
    def delete_materials_sub_playlist(self, playlist_id: UUID, sub_playlist_id: UUID) -> bool:
        """サブプレイリストを削除"""
        return self.materials_sub_playlist_repository.delete(playlist_id, sub_playlist_id)

    def search_materials_sub_playlists(
        self,
        playlist_id: UUID,
        title: str | None = None,
        phase: str | None = None,
        ...
        """サブプレイリストを検索"""
        return self.materials_sub_playlist_repository.search(
...
class MaterialsVideoService:
    ...
    def _validate_sub_playlist_belongs_to_playlist(self, playlist_id: UUID, sub_playlist_id: UUID) -> None:
        """サブプレイリストが指定されたプレイリストに属しているかを検証"""
        sub_playlist = self.materials_sub_playlist_repository.find_by_id(playlist_id, sub_playlist_id)
        if not sub_playlist:
            raise APIException(ErrorMessage.SUB_PLAYLIST_NOT_FOUND)

    def get_all_materials_videos(self, playlist_id: UUID, sub_playlist_id: UUID) -> list[dict[str, Any]]:
        """指定されたサブプレイリストのビデオを取得"""
        self._validate_sub_playlist_belongs_to_playlist(playlist_id, sub_playlist_id)
        return self.materials_video_repository.find_all(sub_playlist_id)

    def get_materials_video_by_id(self, playlist_id: UUID, sub_playlist_id: UUID, video_id: UUID) -> dict[str, Any]:
        """指定したIDのビデオを取得"""
        self._validate_sub_playlist_belongs_to_playlist(playlist_id, sub_playlist_id)
        return self.materials_video_repository.find_by_id(sub_playlist_id, video_id)

    def create_materials_video(self, playlist_id: UUID, sub_playlist_id: UUID, video_data: dict[str, Any]) -> dict[str, Any]:
        """ビデオを作成"""
        self._validate_sub_playlist_belongs_to_playlist(playlist_id, sub_playlist_id)
        return self.materials_video_repository.create(sub_playlist_id, video_data)

    def update_materials_video(self, playlist_id: UUID, sub_playlist_id: UUID, video_id: UUID, video_data: dict[str, Any]) -> dict[str, Any]:
        """ビデオを更新"""
        self._validate_sub_playlist_belongs_to_playlist(playlist_id, sub_playlist_id)
        return self.materials_video_repository.update(sub_playlist_id, video_id, video_data)

    def delete_materials_video(self, playlist_id: UUID, sub_playlist_id: UUID, video_id: UUID) -> bool:
        """ビデオを削除"""
        self._validate_sub_playlist_belongs_to_playlist(playlist_id, sub_playlist_id)
        return self.materials_video_repository.delete(sub_playlist_id, video_id)

    def search_materials_videos(
        self,
        playlist_id: UUID,
        sub_playlist_id: UUID,
        title: str | None = None,
        ...
        """ビデオを検索"""
        self._validate_sub_playlist_belongs_to_playlist(playlist_id, sub_playlist_id)
        return self.materials_video_repository.search(
...
class MaterialsFavoriteService:
    ...
    def get_all_materials_favorites(self) -> list[dict[str, Any]]:
        """すべてのお気に入りを取得"""
        return self.materials_favorite_repository.find_all()

    def get_materials_favorite_by_id(self, favorite_id: UUID) -> dict[str, Any]:
        """指定したIDのお気に入りを取得"""
        return self.materials_favorite_repository.find_by_id(favorite_id)

    def get_favorites_by_user_id(self, user_id: UUID) -> list[dict[str, Any]]:
        """指定したユーザーIDのお気に入り一覧を取得"""
        return self.materials_favorite_repository.find_by_user_id(user_id)

    def is_favorited(self, user_id: UUID, video_id: UUID) -> bool:
        """指定したユーザーがビデオをお気に入り登録しているかチェック"""
        favorite = self.materials_favorite_repository.find_by_user_id_and_video_id(user_id, video_id)
        return favorite is not None

    def create_materials_favorite(self, favorite_data: dict[str, Any]) -> dict[str, Any]:
        """お気に入りを作成"""
        # 既に存在する場合はエラーを返す
        user_id = favorite_data.get("user_id")
        video_id = favorite_data.get("video_id")
        if user_id and video_id:
            existing = self.materials_favorite_repository.find_by_user_id_and_video_id(
        ...
        return self.materials_favorite_repository.create(favorite_data)

    def toggle_favorite(self, user_id: UUID, video_id: UUID) -> dict[str, Any]:
        """お気に入りの追加/削除を切り替え"""
        existing = self.materials_favorite_repository.find_by_user_id_and_video_id(user_id, video_id)

        if existing:
            # 既に存在する場合は削除
            self.materials_favorite_repository.delete_by_user_id_and_video_id(user_id, video_id)
            ...
            favorite = self.materials_favorite_repository.create(favorite_data)
            return {"is_favorited": True, "favorite": favorite, "message": "お気に入りに追加しました"}

    def delete_materials_favorite(self, user_id: UUID, video_id: UUID) -> bool:
        """指定したユーザーIDとビデオIDのお気に入りを削除"""
        return self.materials_favorite_repository.delete_by_user_id_and_video_id(user_id, video_id)

    def update_materials_favorite(self, favorite_id: UUID, favorite_data: dict[str, Any]) -> dict[str, Any]:
        """お気に入りを更新"""
        return self.materials_favorite_repository.update(favorite_id, favorite_data)

    def delete_materials_favorite_by_id(self, favorite_id: UUID) -> bool:
        """お気に入りを削除（ID指定）"""
        return self.materials_favorite_repository.delete(favorite_id)

    def get_favorite_videos_with_details(self, user_id: UUID) -> list[dict[str, Any]]:
        """指定したユーザーIDのお気に入り動画とその関連情報（プレイリスト、サブプレイリスト）を取得"""
        return self.materials_favorite_repository.find_favorite_videos_with_details(user_id)
