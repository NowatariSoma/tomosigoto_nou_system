...
def get_instructor_candidates(
    ...
    candidates = session_instructor_service.get_instructor_candidates(practice_schedule_id)
    return candidates
...
def get_session_instructors(
    ...
    """セッション指導者一覧を取得"""
    result = session_instructor_service.get_all_session_instructors_simple(
...
def get_session_instructor(
    ...
    """指定したIDのセッション指導者を取得"""
    session_instructor = session_instructor_service.get_session_instructor(session_instructor_id)
    return session_instructor


@router.get("/schedule/{schedule_id}", response_model=list[SessionInstructorResponse])
def get_session_instructors_by_schedule(
    schedule_id: UUID,
    #current_user: CurrentUser = Depends(get_current_user),
    session_instructor_service: SessionInstructorService = Depends(get_session_instructor_service),
    current_user: CurrentUser = Depends(require_member_or_above),
    ...
    """指定したスケジュールの指導者一覧を取得"""
    session_instructors = session_instructor_service.get_session_instructors_by_schedule(schedule_id)
    return session_instructors


@router.get("/schedule/{schedule_id}/slot/{slot_order}", response_model=list[SessionInstructorResponse])
def get_session_instructors_by_schedule_and_slot(
    schedule_id: UUID,
    slot_order: int,
    #current_user: CurrentUser = Depends(get_current_user),
    session_instructor_service: SessionInstructorService = Depends(get_session_instructor_service),
    ...
    session_instructors = session_instructor_service.get_session_instructors_by_schedule_and_slot(
...
def get_session_instructors_by_attendance(
    attendance_id: UUID,
    #current_user: CurrentUser = Depends(get_current_user),
    session_instructor_service: SessionInstructorService = Depends(get_session_instructor_service),
    current_user: CurrentUser = Depends(require_member_or_above),
    ...
    session_instructors = session_instructor_service.get_session_instructors_by_attendance(attendance_id)
    return session_instructors
...
def create_session_instructor(
    ...
    """セッション指導者を作成"""
    session_instructor = session_instructor_service.create_session_instructor(
...
def create_session_instructors_bulk(
    ...
    """セッション指導者を一括作成"""
    result = session_instructor_service.create_session_instructors_bulk(
...
def update_session_instructor(
    ...
    session_instructor = session_instructor_service.update_session_instructor(
...
def delete_session_instructor(
    ...
    try:
        success = session_instructor_service.delete_session_instructor(session_instructor_id)
...
def delete_session_instructors_by_schedule(
    schedule_id: UUID,
    #current_user: CurrentUser = Depends(get_current_user),
    session_instructor_service: SessionInstructorService = Depends(get_session_instructor_service),
    current_user: CurrentUser = Depends(require_instructor_or_admin),
    ...
    deleted_count = session_instructor_service.delete_session_instructors_by_schedule(schedule_id)
...
def delete_session_instructors_by_schedule_and_slot(
    schedule_id: UUID,
    slot_order: int,
    #current_user: CurrentUser = Depends(get_current_user),
    session_instructor_service: SessionInstructorService = Depends(get_session_instructor_service),
    ...
    deleted_count = session_instructor_service.delete_session_instructors_by_schedule_and_slot(
        schedule_id, slot_order
    )
...
def move_session_instructor(
    ...
    """インストラクターを別の会場・時限に移動"""
    updated_session_instructor = session_instructor_service.move_session_instructor(
