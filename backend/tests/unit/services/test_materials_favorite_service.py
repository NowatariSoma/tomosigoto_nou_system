...
class TestMaterialsFavoriteService:
    ...
    @pytest.mark.asyncio
    async def test_get_favorite_by_id(self):
        """ID指定でお気に入り取得"""
        favorite_id = uuid4()
        favorite = make_materials_favorite(id=str(favorite_id))
        self.mock_repo.find_by_id.return_value = favorite

        result = await self.service.get_materials_favorite_by_id(favorite_id)
    ...
    @pytest.mark.asyncio
    async def test_is_favorited_true(self):
        """お気に入りチェック - 登録済み"""
        user_id = uuid4()
        video_id = uuid4()
        self.mock_repo.find_by_user_id_and_video_id.return_value = make_materials_favorite()

        result = await self.service.is_favorited(user_id, video_id)

        assert result is True

    @pytest.mark.asyncio
    async def test_is_favorited_false(self):
        """お気に入りチェック - 未登録"""
        self.mock_repo.find_by_user_id_and_video_id.return_value = None

        result = await self.service.is_favorited(uuid4(), uuid4())
    ...
    @pytest.mark.asyncio
    async def test_create_favorite_success(self):
        """お気に入り作成 - 正常"""
        user_id = str(uuid4())
        video_id = str(uuid4())
        ...
        result = await self.service.create_materials_favorite(data)

        assert result == created

    @pytest.mark.asyncio
    async def test_create_favorite_already_exists(self):
        """お気に入り作成 - 既に存在"""
        user_id = str(uuid4())
        video_id = str(uuid4())
        data = {"user_id": user_id, "video_id": video_id}
        ...
        with pytest.raises(APIException):
            await self.service.create_materials_favorite(data)
    ...
    async def test_toggle_favorite_add(self):
        ...
        result = await self.service.toggle_favorite(user_id, video_id)
    ...
    async def test_delete_favorite(self):
        ...
        result = await self.service.delete_materials_favorite(user_id, video_id)
    ...
    @pytest.mark.asyncio
    async def test_get_favorite_videos_with_details(self):
        """お気に入り動画詳細取得"""
        user_id = uuid4()
        details = [{"id": str(uuid4()), "title": "お気に入り動画"}]
        self.mock_repo.find_favorite_videos_with_details.return_value = details

        result = await self.service.get_favorite_videos_with_details(user_id)
