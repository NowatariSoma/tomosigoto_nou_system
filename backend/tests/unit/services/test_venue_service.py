...
class TestVenueService:
    ...
    @pytest.mark.asyncio
    async def test_get_venue_success(self):
        """ID指定で会場取得 - 正常"""
        venue_id = uuid4()
        venue = make_venue(id=str(venue_id))
        self.mock_repo.find_by_id.return_value = venue

        result = await self.service.get_venue(venue_id)
    ...
    @pytest.mark.asyncio
    async def test_create_venue_success(self):
        """会場作成 - 正常"""
        ...
        result = await self.service.create_venue(venue_data)
    ...
    @pytest.mark.asyncio
    async def test_update_venue_success(self):
        """会場更新 - 正常"""
        venue_id = uuid4()
        ...
        result = await self.service.update_venue(venue_id, update_data)
    ...
    @pytest.mark.asyncio
    async def test_remove_venue_success(self):
        """会場削除 - 正常"""
        venue_id = uuid4()
        venue = make_venue(id=str(venue_id))
        self.mock_repo.find_by_id.return_value = venue
        ...
        result = await self.service.remove_venue(venue_id)
    ...
    @pytest.mark.asyncio
    async def test_remove_venue_not_found(self):
        """会場削除 - 存在しない場合はAPIException"""
        venue_id = uuid4()
        self.mock_repo.find_by_id.return_value = None

        with pytest.raises(APIException):
            await self.service.remove_venue(venue_id)
