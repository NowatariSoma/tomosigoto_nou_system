...
class TestPartService:
    ...
    def test_get_all_parts(self):
        ...
        result = self.service.get_all_parts()
    ...
    def test_get_part_success(self):
        ...
        result = self.service.get_part(part_id)
    ...
    def test_get_part_not_found(self):
        ...
        with pytest.raises(APIException):
            self.service.get_part(uuid4())

    # ===== get_active_parts =====

    def test_get_active_parts(self):
        """アクティブパートのみ取得"""
        parts = [make_part(status="active")]
        self.mock_repo.find_active.return_value = parts

        result = self.service.get_active_parts()
    ...
    def test_create_part_success(self):
        ...
        result = self.service.create_part(part_data)

        assert result == created

    def test_create_part_stage_not_found(self):
        """パート作成 - 舞台が存在しない"""
        stage_id = str(uuid4())
        part_data = {"name": "新パート", "stage_id": stage_id}
        ...
        with pytest.raises(APIException):
            self.service.create_part(part_data)

        self.mock_repo.create.assert_not_called()

    def test_create_part_invalid_stage_id(self):
    ...
    def test_create_part_duplicate_name(self):
        """パート作成 - 同名パートが存在する"""
        stage_id = str(uuid4())
    ...
    def test_create_part_without_stage_id(self):
        """パート作成 - stage_idなし"""
        part_data = {"name": "新パート"}
    ...
    def test_update_part_success(self):
        ...
        result = self.service.update_part(part_id, update_data)
    ...
    def test_update_part_not_found(self):
        ...
        with pytest.raises(APIException):
            self.service.update_part(uuid4(), {"name": "新名"})

    def test_update_part_empty_data(self):
        """パート更新 - 空データの場合は既存を返す"""
        part_id = uuid4()
        ...
        result = self.service.update_part(part_id, {"name": None})

        assert result == existing
        self.mock_repo.update.assert_not_called()

    def test_update_part_duplicate_name(self):
        """パート更新 - 名前が重複する場合"""
        part_id = uuid4()
        ...
        with pytest.raises(APIException):
            self.service.update_part(part_id, {"name": "既存名"})

    # ===== remove_part =====

    def test_remove_part_success(self):
        """パート削除 - 正常"""
        part_id = uuid4()
        self.mock_repo.find_by_id.return_value = make_part(id=str(part_id))
        self.mock_repo.delete.return_value = None

        result = self.service.remove_part(part_id)
    ...
    def test_remove_part_not_found(self):
        ...
        with pytest.raises(APIException):
            self.service.remove_part(uuid4())

    # ===== deactivate_part / activate_part =====

    def test_deactivate_part_success(self):
        """パート非アクティブ化"""
        part_id = uuid4()
        ...
        result = self.service.deactivate_part(part_id)

        assert result["status"] == "inactive"
        self.mock_repo.update.assert_called_once_with(part_id, {"status": "inactive"})

    def test_activate_part_success(self):
        """パートアクティブ化"""
        part_id = uuid4()
        ...
        result = self.service.activate_part(part_id)

        assert result["status"] == "active"

    # ===== get_parts_by_stage_id =====

    def test_get_parts_by_stage_id_success(self):
        """舞台IDでパート取得 - 正常"""
        stage_id = str(uuid4())
        ...
        result = self.service.get_parts_by_stage_id(stage_id)

        assert result == parts

    def test_get_parts_by_stage_id_not_found(self):
        """舞台IDでパート取得 - 舞台が存在しない"""
        stage_id = str(uuid4())
        self.mock_stage_repo.exists.return_value = False

        with pytest.raises(APIException):
            self.service.get_parts_by_stage_id(stage_id)

    def test_get_parts_by_stage_id_invalid(self):
        """舞台IDでパート取得 - 無効なUUID"""
        with pytest.raises(APIException):
            self.service.get_parts_by_stage_id("not-a-uuid")
    ...
    def test_get_part_count(self):
        ...
        result = self.service.get_part_count()
