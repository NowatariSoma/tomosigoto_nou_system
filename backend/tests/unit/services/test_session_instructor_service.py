...
class TestSessionInstructorService:
    ...
    def test_get_all_session_instructors_paginated(self):
        ...
        result = self.service.get_all_session_instructors(page=1, per_page=20)
    ...
    def test_get_all_session_instructors_with_filters(self):
        """セッション指導者一覧取得 - フィルタ付き"""
        schedule_id = uuid4()
        self.mock_repo.find_all_with_details.return_value = []
        self.mock_repo.count_all.return_value = 0

        result = self.service.get_all_session_instructors(
    ...
    def test_get_session_instructor_success(self):
        """セッション指導者取得 - 正常"""
        instructor_id = uuid4()
        instructor = make_session_instructor(id=str(instructor_id))
        self.mock_repo.find_by_id.return_value = instructor

        result = self.service.get_session_instructor(instructor_id)

        assert result == instructor

    def test_get_session_instructor_not_found(self):
        """セッション指導者取得 - 存在しない"""
        self.mock_repo.find_by_id.return_value = None

        with pytest.raises(APIException):
            self.service.get_session_instructor(uuid4())

    # ===== create_session_instructor =====

    def test_create_session_instructor_success(self):
        """セッション指導者作成 - 正常"""
        schedule_id = uuid4()
        attendance_id = uuid4()
        data = {
        ...
        result = self.service.create_session_instructor(data)

        assert result == created

    def test_create_session_instructor_schedule_not_found(self):
        """セッション指導者作成 - スケジュールなし"""
        data = {
            "schedule_id": uuid4(),
            "attendance_id": uuid4(),
        ...
        with pytest.raises(HTTPException):
            self.service.create_session_instructor(data)

    def test_create_session_instructor_attendance_not_found(self):
        """セッション指導者作成 - 出席記録なし"""
        schedule_id = uuid4()
        data = {
            "schedule_id": schedule_id,
    ...
    def test_create_session_instructor_schedule_mismatch(self):
    ...
    def test_delete_session_instructor_success(self):
        """セッション指導者削除 - 正常"""
        instructor_id = uuid4()
        ...
        result = self.service.delete_session_instructor(instructor_id)

        assert result is True

    def test_delete_session_instructor_not_found(self):
        """セッション指導者削除 - 存在しない"""
        self.mock_repo.find_by_id.return_value = None

        with pytest.raises(APIException):
            self.service.delete_session_instructor(uuid4())
    ...
    def test_delete_by_schedule(self):
        ...
        result = self.service.delete_session_instructors_by_schedule(schedule_id)

        assert result == 3

    # ===== get_all_session_instructors_simple =====

    def test_get_all_session_instructors_simple(self):
        """シンプル版一覧取得"""
        items = [make_session_instructor()]
        self.mock_repo.find_all_with_details.return_value = items

        result = self.service.get_all_session_instructors_simple()
