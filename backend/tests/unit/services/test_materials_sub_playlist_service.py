...
class TestMaterialsSubPlaylistService:
    ...
    def setup_method(self):
        ...
        self.mock_playlist_repo = Mock()
        self.mock_youtube_oauth_token_repo = Mock()
        self.service = MaterialsSubPlaylistService(
            ...
            youtube_oauth_token_repository=self.mock_youtube_oauth_token_repo,
        )

    # ===== get_all_materials_sub_playlists =====

    def test_get_all_sub_playlists(self):
        """サブプレイリスト一覧取得"""
        playlist_id = uuid4()
        ...
        result = self.service.get_all_materials_sub_playlists(playlist_id)

        assert result == sub_playlists

    # ===== get_materials_sub_playlist_by_id =====

    def test_get_sub_playlist_by_id(self):
        """ID指定でサブプレイリスト取得"""
        playlist_id = uuid4()
        ...
        result = self.service.get_materials_sub_playlist_by_id(
    ...
    def test_create_sub_playlist_without_url(self):
        ...
        result = self.service.create_materials_sub_playlist(
    ...
    def test_create_sub_playlist_auto_import_disabled(self):
    ...
    def test_update_sub_playlist_success(self):
        ...
        result = self.service.update_materials_sub_playlist(
            playlist_id, sub_playlist_id, update_data
        )

        assert result["title"] == "更新タイトル"

    def test_update_sub_playlist_not_found(self):
        """サブプレイリスト更新 - 存在しない"""
        playlist_id = uuid4()
        sub_playlist_id = uuid4()
        ...
        with pytest.raises(APIException):
            self.service.update_materials_sub_playlist(
                playlist_id, sub_playlist_id, {"title": "更新"}
            )

    # ===== delete_materials_sub_playlist =====

    def test_delete_sub_playlist(self):
        """サブプレイリスト削除"""
        playlist_id = uuid4()
        sub_playlist_id = uuid4()
        self.mock_sub_playlist_repo.delete.return_value = True

        result = self.service.delete_materials_sub_playlist(
            playlist_id, sub_playlist_id
        )
